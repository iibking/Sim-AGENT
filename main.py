...
async def phase_status(session) -> dict:
    ...
    log.info("Status keys: %s", list(status.keys()))
    ...
    if not enabled:
        log.warning("⚠️  accountEnabled=false — calling agent.enableBrowserAccess")
        email = user.get("email", "")
        if email:
            result = await call_tool(session, "agent.enableBrowserAccess", {"email": email})
            log.info("enableBrowserAccess result: %s", result)
        else:
            log.warning("No email found in session — cannot auto-enable account")
    else:
...
async def phase_bonuses(session) -> list:
    ...
    if signin.get("canClaim"):
        ...
        log.info("Sign-in bonus: not ready yet")
...
async def phase_feed(session) -> tuple:
    trend = await call_tool(session, "agent.concepts.search",
                            {"sort": "trending", "limit": 15})
    trending = trend.get("items", [])
...
async def phase_post(session, trending, billboard, status) -> str | None:
    ...
    log.info("Draft completion_id: %s | keys: %s", completion_id, list(draft.keys()))
