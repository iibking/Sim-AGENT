...
async def heartbeat():
    ...
    async with streamablehttp_client(MCP_URL, headers=headers) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            # Discover all available tools (helps us find follow/unfollow names)
            try:
                tools_result = await session.list_tools()
                tool_names = [t.name for t in tools_result.tools]
                log.info("Available tools (%d): %s", len(tool_names), tool_names)
                follow_tools = [t for t in tool_names if "follow" in t.lower() or "social" in t.lower()]
                log.info("Follow-related tools: %s", follow_tools)
            except Exception as e:
